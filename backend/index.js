...
// * Endpoint to create a new CSV and convert it to json
app.post("/api/files", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "An error ocurred while upload the csv" });
  }
...
// * Endpoint to filter and send especific data to the user
app.get("/api/users", async (req, res) => {
  const { q } = req.query;

  if (!q)
    return res.status(500).json({ message: "The query param q is required" });
  ...
  // * Filter the data that coincide with the q param
